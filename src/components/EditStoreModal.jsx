...
const EditStoreModal = ({ store, onClose, onSuccess }) => {
  ...
  // Populate form with store data when component mounts
  useEffect(() => {
    if (store) {
      setFormData({
        name: store.name || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await callApi({
        endpoint: `api/v1/store/update/${store._id}`,
        method: "PATCH",
        body: storeData
      });

      if (response.data && response.data.success) {
        setSuccess(`Store "${formData.name}" updated successfully!`);
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div className="flex-1">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-200">Edit Store</h2>
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-1">Update store information</p>
          </div>
          <button
            onClick={onClose}
